...
Determine if the system in compliance with a regulatory limit of 2.5 kg/(1000 m$^3$). You can do this analytically or computationally.
:::
...
- $A$ and $B$ are coefficients from the linearization of outgoing radiation physics, which are determined by assuming that the pre-industrial temperature of $14^\circ\text{C}$ was stable without anthropogenic greenhouse gas emissions, and have corresponding values $B=1.3\ \text{W}/\text{m}^2/^\circ\text{C}$ and $$A=\frac{(1-\alpha)S}{4} + 14B.$$
...
Let $\alpha_i = 0.5$ and $\alpha_0 = 0.3$. Plot $\alpha$ versus $T$ for values of $T$ between $-20^\circ\text{C}$ and $20^\circ\text{C}$.
...
Simulate the simple climate model using the Neoprotereozoic value of $S$ with temperature-varying albedo for initial values of $T$ spanning $-60^\circ\text{C} \leq T_0 \leq 30^\circ\text{C}$ over a period of 200 years. Plot the temperature trajectories. How many equilibria are there and what are their stabilities?
:::
